...
image::kafka/secret-create-from-yaml.png[alt text, 640]
...
stringData:
  ...
  KAFKA_USERNAME: <Kafka SA Client ID>
  KAFKA_PASSWORD: <Kafka SA Client Secret>
  KAFKA_TOPIC_IMAGES: images
  KAFKA_TOPIC_OBJECTS: objects
----

image::kafka/secret-data.png[alt text, 640]

image::kafka/secret-created.png[alt text, 640]

image::kafka/secret-add-workload.png[alt text, 640]

image::kafka/secret-in-app.png[alt text, 640]
